...
| 순위 | 할 일 | 상태 |
|---|---|---|
| 1 | **`persona.py` 되묻기 조항 정리** | MLX 측정으로 근거 확보(3/6→5/6). 모델과 무관한 한 벌로. `feature/mlx-brain`. ⚠️ 8/5 기록처 수정으로 조항이 3줄 더 늘었다 — 정리할 때 **다시 측정**할 것 |
| 2 | **MLX를 기본 뇌로** | 프롬프트 정리 뒤. `.env`의 `VLLM_BASE_URL`+`VLLM_API_KEY=sk-optiq-local`만 |
