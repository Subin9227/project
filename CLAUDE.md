...
디스코드 집사봇. 사용자가 자연어로 말하면 노션 등 기록처를 대신 채워주는 비서. '닛몰캐쉬 잘자요 아가씨' 말투로 사용자를 "아가씨"라 부른다.
...
| `agent.py` | `create_react_agent` (사람별 모델 + 도구 18개 + SqliteSaver 기억) |
...
**도구 18개** = 루틴 5(조회/**읽기**/체크/쓰기/사진) + 과제 4 + 블레이버스 9

## 노션 연결 (중요)

- **REST 직접 호출뿐**이다. MCP 서버는 #9 Phase 1에서 걷어냈다.   
  이유 ① 스키마 24개가 매 요청에 실려 비쌌고    
  ② 토큰이 subprocess 실행 시점에 고정돼 사용자별 노션이 불가능했다   
  ③ 작은 모델이 24개 중에서 잘 못 골랐다.  
- ⚠️ **행 구조를 코드에 박지 않는다.** 행을 만들 때 그 DB의 **기본 템플릿을 복사**한다 (`_copy_template`).    
  노션에서 템플릿을 고치면 봇이 따라온다 — 실제로 작업 중 '오늘 한 일' 헤딩이 하나 늘었고, 하드코딩이었으면 코드를 고쳐야 했다.  
- 데일리루틴 데이터소스: `a5a0ffe9-306d-82c0-b5df-07a9145e578b` (주인 기본값) ("비서 전용 페이지" 안 "오프라인 데일리 루틴 (2)". config.py `NOTION_ROUTINE_DS_ID`)   
...
- 🔴 **체크박스 이름과 '달성률'만 코드에 박혀 있다** (`notion_tools.ITEMS`, `props["달성률"]`).  
  ...
  사용자가 노션에서 `운동`→`헬스`로 바꾸면 그 사람의 루틴 기능이 통째로 죽고, 체크박스를 새로 만들어도 봇은 모른다.   
  달성률 이름을 바꾸면 주간 브리핑 합계가 0이 된다.   
  · 지금(2026-08-05)은 **방어만** 했다: `_set_checkbox()`가 400을 받으면 실제 있는 체크박스 목록을 보여준다(`ItemMissing`).
    죽지는 않지만 여전히 못 켠다.
  · ⏭️ **근본은 `ITEMS`를 버리고 DB 스키마에서 체크박스 속성을 읽어오는 것.**
    헤딩을 다루는 방식과 같아진다.   
    `ALIASES`('코딩'→'코테') 같은 편의는 따로 남겨야 한다.   
...
- ⚠️ 페이지 URL의 id는 **database_id**이고 우리가 쓰는 건 **data_source_id**다.  
  다른 값이라 `GET /databases/{id}` 로 한 단계 더 물어봐야 한다 (`onboarding.discover_databases`).
...
- `/register`(모달, 비밀만) → `/setup`(슬래시 인자, 취향) → `/status` → `/forget`  
  ⚠️ `/setup`은 **준 인자만 갱신**한다. 
  `target`은 비우면 그대로 두고 `여기`(이 채널)·`끄기`(멈춤)를 낱말로 받는다. 기본값으로 현재 채널을 밀어넣으면 본인도 모르게 알림이 등록된다(2026-08-04 사고)    
  ⚠️ **`끄기`는 지우기가 아니라 멈춤**이다(`alarm_off='1'`).
  받을 곳·시각은 남는다 — 지웠더니 시각만 다시 넣은 사람이 영영 알림을 못 받았다(2026-08-05). 받을 곳을 정하면 `alarm_off`가 자동으로 풀린다  
  ⚠️ 요일은 **드롭다운(`app_commands.Choice`)으로만** 받는다. 손으로 치면 `wen` 같은 오타가 나는데 `_parse_weekly`가 조용히 월요일로 떨어뜨렸다. 시각은 저장 전에 `parse_clock()`이 `HH:MM`을 강제한다 — `8시40분`이 저장되면 한참 뒤 알림 루프에서 터진다.   
  ⚠️ 디스코드 모달은 **입력칸 최대 5개**라 자격증명만으로 꽉 찬다. 그래서 둘로 나눴다.  
- 저장소 `data/users.sqlite` (memory.sqlite와 **파일 분리** — 그쪽은 시작 시 통째로 지운다)  
  ⚠️ 칸을 더할 땐 `_COLUMNS`에 넣기만 하면 된다 — `_connect()`가 열 때마다 빠진 칸을 `ALTER TABLE`로 채운다.   
  `CREATE TABLE IF NOT EXISTS`는 **이미 있는 표에 칸을 안 늘려서**, 이게 없으면 기존 사용자 행이 `User(**data)`에서 통째로 터진다.  
- ⚠️ 블레이버스는 OAuth가 없어 **재사용 가능한 비밀번호를 들고 있어야** 한다. Fernet은 공격자가 아니라 `cat users.sqlite`를 막는 것이다.  
  서버가 털리면 `.env`의 `CRED_KEY`도 같이 털린다 → **등록 모달에 그 사실을 한국어로 고지**하고 `/forget`을 둔다.  
- ⚠️ `CRED_KEY`가 바뀌면 옛 암호문을 못 푼다. 그때 **그 사람만 건너뛰고 봇은 살아남는다** (안 그러면 `all_users()`가 죽고 알림 루프가 죽고 봇이 아예 못 뜬다)
...
- 2026-08-05  **노션 글을 읽는 도구가 없었다** → `routine_read` 신설 (`feature/mlx-brain`, 미커밋)
    - 증상: "오늘 회고에 뭐라고 썼어?"에 답을 못 한다. Qwen은 "직접 확인할 수 없어요"라고 하고,
      Claude는 답을 하는데 **노션을 안 보고 한다**
    - 🔴 원인: **읽기 도구가 처음부터 없었다.** `routine_today`는 체크박스와 달성률만 준다.
      Claude가 답한 건 `routine_write`로 **자기가 방금 써넣은 내용을 대화기억에서 꺼낸 것**
      (memory.sqlite에서 확인. 스스로 "제가 방금 적은 내용을 다시 말씀드릴게요"라고 밝힌다)
      → 다른 사람 대화엔 그 기억이 없으니 못 한다. **모델 문제가 아니었다**
    - ⚠️ 8/4에 넣은 `[원래 이 칸에 있던 내용]`이 유일한 읽기 경로였는데 **`routine_write` 안에
      갇혀 있었다.** 어제 '회고 축약'이 됐던 건 그게 쓰기 요청이라 그 문을 지났기 때문이다.
      "확인만 해줘"는 쓰기가 아니라 문이 없다
    - `_section_lines()` 신설 — ⚠️ `_section_blocks`와 달리 **하위 칸까지** 훑는다.
      '회고'(h2)는 자기 밑이 비어 있고 글은 전부 h3('오늘 한 일' 등)에 있어서,
      다음 헤딩에서 멈추면 **늘 '비어 있음'**이 나온다 → 같거나 위 단계 헤딩까지 간다
    - ⚠️ 읽기 도구는 `_find_or_create_row`를 **부르면 안 된다.** "뭐 적었지?" 한 마디에
      빈 행이 생긴다 → `_find_row`만 쓰고, 셀프테스트가 소스에서 그걸 확인한다
    - `routine_today` 결과 끝에 "(칸에 적힌 글을 보려면 routine_read)"를 달았다.
      길을 안 알려주면 모델이 그 결과만 보고 기억으로 지어낸다
    - 도구 **17개 → 18개**. 9B 재측정: 원래 6케이스 15/30(50%), 직전 18/30(60%)보다 낮지만
      **차이 3은 이 프로브의 흔들림 범위**라 도구가 늘어서인지 단정 못 함.
      새 케이스 '오늘 회고에 뭐라고 썼지?'는 **4/5로 `routine_read`를 제대로 고른다**
