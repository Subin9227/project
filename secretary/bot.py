...
대화기억의 핵심:
    thread_id = "디스코드 사용자 ID:KST 오늘 날짜".
    채널이 아니라 사람이 기준이다 — 채널로 묶으면 한 채널의 두 사람이 서로의
    대화기억과 도구 결과를 보게 된다.
    같은 사람이라도 날짜가 바뀌면 다른 thread_id가 되므로, 대화는 하루 단위로
...
async def _prune_old_threads(checkpointer: AsyncSqliteSaver) -> int:
    ...
    시작할 때 1회. 프로세스가 몇 달씩 안 죽으면 그때 main()의
